...
|DEPENDABOT_DEPENDENCY_GROUPS|vNext|**_Optional_**. The list of dependency group rules, in JSON format. For example: `[{"name":"microsoft","rules":{"patterns":["Microsoft.*"]}}]`. See [official docs](https://docs.github.com/en/code-security/dependabot/dependabot-version-updates/configuration-options-for-the-dependabot.yml-file#groups) for more. |
...
|DEPENDABOT_MESSAGE_HEADER|Update,<br/>vNext|**_Optional_**. Additional pull request description text to shown before the dependency change info.|
|DEPENDABOT_MESSAGE_FOOTER|Update,<br/>vNext|**_Optional_**. Additional pull request description text to shown after the dependency change info. This text will not be truncated, even when the dependency change info exceeds the PR maximum description length. |
