...
|targetMilestoneIds|**_Optional_**. The IDs of the milestones to target. If this value is not supplied then the milestone from `dependabot.yml` file would be used, if defined. Useful if one needs to have a dynamic list of milestones. These could be also a work item IDs.|
