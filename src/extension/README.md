...
|autoApprove|**_Optional_**. Determines if the pull requests that dependabot creates should be automatically completed. When set to `true`, pull requests will be approved automatically by the user specified in `autoApproveUser` field.|
|autoApproveUser|**_Optional_**. Email of the user that should be used to automatically approved. Only taken into account if `autoApprove` is set to `true`.|
|autoApproveToken|**_Optional_**. A personal access token that is assigned to the user specified in `autoApproveUser` to automatically approve the created PR. Only taken into account if `autoApprove` is set to `true`.|
