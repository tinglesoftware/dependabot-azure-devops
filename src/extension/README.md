...
|autoApprove|**_Optional_**. Determines if the pull requests that dependabot creates should be automatically completed. When set to `true`, pull requests will be approved automatically by the user specified in `autoApproveUserEmail` field.|
|autoApproveUserEmail|**_Optional_**. Email of the user that should be used to automatically approved. Only taken into account if `autoApprove` is set to `true`.|
|autoApproveUserToken|**_Optional_**. A personal access token that is assigned to the user specified in `autoApproveUserEmail` to automatically approve the created PR. Only taken into account if `autoApprove` is set to `true`.|
...
variables:
  DEPENDABOT_DOCKER_IMAGE_TAG: '0.3.0' # could also be 'latest'
