...
internal static class CollectionExtensions
{
    ...
    /// <summary>
    /// Tries to add an element with the provided key and value to the <see cref="IDictionary{TKey, TValue}"/>,
    /// provided the value is not equal to the type's default value (or empty for strings).
    /// </summary>
    /// <typeparam name="TKey">The type of keys in the dictionary.</typeparam>
    /// <typeparam name="TValue">The type of values in the dictionary.</typeparam>
    ...
    /// <exception cref="ArgumentNullException">key is null.</exception>
    /// <returns></returns>
    public static bool TryAddIfNotDefault<TKey, TValue>(this IDictionary<TKey, TValue> dictionary, TKey key, TValue? value)
        where TKey : notnull
    {
        if (value is not null || (value is string s && !string.IsNullOrWhiteSpace(s)))
        {
